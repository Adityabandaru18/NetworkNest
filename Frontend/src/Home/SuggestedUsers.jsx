...
const SuggestedUsers = ({ name, user_image }) => {
  ...
  return (
    <>
      {name && (
        <div className='grid grid-cols-2 items-center mx-auto my-3'>
          <div className='flex items-baseline'>
            ...
            <div>
              <p className="inline-block ml-3 font-semibold text-base text-[20px]" onClick={handleClick}>{name}</p>
            </div>
          </div>
          <div className='text-center relative top-2'>
            {!token && <LuUserPlus className='text-xl absolute right-6 bottom-[0.1px] hover:text-indigo-900 cursor-wait font-light' onClick={handleClick} />}
            {token && ( 
              showPlusIcon ? (
                <LuUserPlus className='text-xl absolute right-6 bottom-[0.1px] hover:text-indigo-900 cursor-pointer' onClick={handleClick} />
              ) : (
      ...
      {showAlert && ( 
        <Alert status='success' variant='solid' position='fixed' top="2%" left='50%' transform='translate(-50%, -50%)' transition='transform 0.5s ease-in-out'>
          <AlertIcon />
